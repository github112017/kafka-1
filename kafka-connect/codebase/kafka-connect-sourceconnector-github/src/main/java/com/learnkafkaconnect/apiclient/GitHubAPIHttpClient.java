...
public class GitHubAPIHttpClient {
    ...
    public GitHubSourceConnectorConfig getConfig() {
        return config;
    }

    public void setConfig(GitHubSourceConnectorConfig config) {
        this.config = config;
    }
    ...
    public JSONArray getNextIssues(Integer page, Instant since) throws InterruptedException {
        ...
        try {
            ...
            log.info("Response Code : " + jsonResponse.getStatusText());

            // deal with headers in any case
            Headers headers = jsonResponse.getHeaders();
            switch (jsonResponse.getStatus()){
                case 200:
                    log.info("Response Array Size : " + jsonResponse.getBody().getArray().length());
